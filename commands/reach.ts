...
function gradeColor(score) {
  ...
  if (score >= 70) return chalk.yellowBright;
  if (score >= 55) return chalk.hex("#f6c177");
  return chalk.redBright;
}

function priorityColor(priority) {
  if (priority === "HIGH") return chalk.redBright;
  if (priority === "MEDIUM") return chalk.yellowBright;
...
function platformHeader(platform) {
  if (platform === "x") return chalk.cyanBright.bold("𝕏 Reach Analysis");
...
function renderSuggestions(suggestions) {
  ...
  return suggestions.map((suggestion, index) => (
    `${index + 1}. ${priorityColor(suggestion.priority)(`[${suggestion.priority}]`)} ${suggestion.issue}\n` +
    `   Fix: ${suggestion.fix}\n` +
...
function renderPlaybookSteps(playbook, limit) {
  ...
  return steps.slice(0, limit).map((step, index) => (
    `${index + 1}. ${chalk.white(step.minuteRange)} ${step.action}\n   ${chalk.gray(step.why)}`
  ));
}
...
function analyzeAction(draft, opts) {
  ...
  try {
    ...
    if (spinner) spinner.succeed("Reach analysis ready.");
    ...
    if (spinner) spinner.fail("Reach analysis failed.");
    throw error;
  }
}
