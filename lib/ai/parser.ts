...
function inferAction(text, url, phone) {
  ...
  if (
    lower.includes('page') &&
    (lower.includes('list') || lower.includes('show') || lower.includes('what are') || lower.includes('do i have'))
  ) return 'query_pages';
...
/**
 * Deterministic parser retained for explicit non-AI parsing paths and tests.
 * @param {string} text
 * @returns {ParsedIntent}
 */
...
function resolveParserProvider(cfg = {}) {
...
function resolveParserApiKey(provider, cfg = {}) {
...
async function aiParseIntent(text, cfg = {}) {
  ...
  if (!input) {
    throw new Error('Missing intent text.');
  }
  const ai = ensureParserApiKey(cfg);
  logDebug(debug, `AI parser credential check passed for provider "${ai.provider}".`);

  const parsed = deterministicParse(input);
  if (!parsed) {
    throw new Error('Unable to map this request confidently. Rephrase and try again.');
  }
  const rawAction = String(parsed.action || '').trim().toLowerCase();
  if (!rawAction || rawAction === 'unknown_input') {
    throw new Error('Unable to map this request confidently. Rephrase and try again.');
  }

  const normalized = normalizeIntent(parsed);
  if (normalized.action === 'query_me' && rawAction !== 'query_me') {
    throw new Error('Unable to map this request confidently. Rephrase and try again.');
  }
