...
function withRuntimeMetadata(started, counters, cost, runtimeLogs) {
  return {
    ...baseMetadata(started, counters.apiCalls, cost),
    runtimeLogs: Array.isArray(runtimeLogs) ? runtimeLogs.slice(-80) : []
  };
}
...
function loadIdempotencyStore() {
  const file = idempotencyStorePath();
...
function normalizeForIdempotency(intent) {
  const clone = { ...(intent || {}) };
  delete clone.confidence;
  delete clone.intentId;
  delete clone.allowReplay;
  return clone;
}

function buildIdempotencyKey(intent) {
  return createHash('sha256').update(stableStringify(normalizeForIdempotency(intent))).digest('hex');
}
...
function shouldUseIdempotency(action) {
  return WRITE_ACTIONS.has(String(action || '').trim());
}

function idempotencyWindowMs() {
  const parsed = Number(process.env.SOCIAL_IDEMPOTENCY_WINDOW_MS || '');
  if (Number.isFinite(parsed) && parsed > 0) return parsed;
  return 15 * 60 * 1000;
}

function tryReserveIdempotency(intent, runtimeLogs) {
  const now = Date.now();
  const windowMs = idempotencyWindowMs();
  const key = buildIdempotencyKey(intent);
  const store = loadIdempotencyStore();

  Object.keys(store).forEach((k) => {
    const at = Number(store[k]?.at || 0);
    if (!at || (now - at) > (windowMs * 4)) delete store[k];
  });

  const existing = store[key];
  if (existing && Number(existing.at || 0) > 0 && (now - Number(existing.at)) <= windowMs) {
    runtimeLogs.push(`idempotency duplicate blocked (${key.slice(0, 12)})`);
  ...
  runtimeLogs.push(`idempotency reserved (${key.slice(0, 12)})`);
...
async function withRetry(label, fn, runtimeLogs, options = {}) {
  const attempts = Math.max(1, Number(options.attempts || 3));
  const baseDelayMs = Math.max(1, Number(options.baseDelayMs || 120));
  let lastError = null;

  for (let i = 1; i <= attempts; i += 1) {
    try {
      if (i > 1) runtimeLogs.push(`${label}: retry ${i}/${attempts}`);
      // eslint-disable-next-line no-await-in-loop
      return await fn();
    } catch (error) {
      lastError = error;
      if (!isTransientError(error) || i >= attempts) break;
      const delay = baseDelayMs * (2 ** (i - 1));
      runtimeLogs.push(`${label}: transient ${error?.code || error?.response?.status || 'unknown'} backoff ${delay}ms`);
      // eslint-disable-next-line no-await-in-loop
      await sleep(delay);
    }
  }
...
async function runPostFacebook(intent, config, counters, runtimeLogs) {
  ...
  const result = await withRetry('post_facebook', () => pageClient.post(`/${ctx.pageId}/feed`, payload), runtimeLogs);
  counters.apiCalls += 1;
  return {
    ...result,
...
async function runPostInstagram(intent, config, counters, runtimeLogs) {
  ...
  const container = await withRetry(
    'post_instagram.create_container',
    () => client.post(`/${igUserId}/media`, {
  ...
  const publish = await withRetry(
    'post_instagram.publish',
    () => client.publishInstagramContainer(igUserId, container.id),
    runtimeLogs
  );
  counters.apiCalls += 1;
...
async function runPostWhatsApp(intent, config, counters, runtimeLogs) {
  ...
  const result = await withRetry('post_whatsapp', () => client.sendWhatsAppMessage(from, payload), runtimeLogs);
  counters.apiCalls += 1;
  return result;
}
...
async function runCreateCampaign(intent, config, counters, runtimeLogs) {
  const token = tokenFor('facebook', config);
  if (!token) throw new Error('No Facebook token found. Run: social auth login -a facebook');
  const act = normalizeAct(intent.accountId || (config?.getDefaultMarketingAdAccountId ? config.getDefaultMarketingAdAccountId() : ''));
  if (!act) {
  ...
  const result = await withRetry(
    'create_campaign',
    () => client.post(`/${act}/campaigns`, payload, {}, { maxRetries: 5 }),
    runtimeLogs
  );
  counters.apiCalls += 1;
  return result;
...
async function runBroadcastMessage(intent, config, counters, runtimeLogs) {
...
async function executeIntent(intent, config) {
  ...
  try {
    ...
    if (shouldUseIdempotency(action) && !runtimeIntent.allowReplay) {
      const lock = tryReserveIdempotency(runtimeIntent, runtimeLogs);
      if (lock.duplicate) {
        return {
          success: true,
          data: {
            status: 'idempotent_skip',
            message: 'Duplicate write action skipped within idempotency window.',
            idempotencyKey: lock.key
          },
          error: null,
          metadata: withRuntimeMetadata(started, counters, null, runtimeLogs)
        };
      }
    }
    ...
    if (action === 'broadcast_message') {
      const data = await runBroadcastMessage(runtimeIntent, config, counters, runtimeLogs);
      return {
        success: true,
        data,
        error: null,
        metadata: withRuntimeMetadata(started, counters, 'depends on conversation category and region', runtimeLogs)
      };
    }
    ...
    if (action === 'publish_post' || action === 'create_ad' || action === 'get_metrics' || action === 'send_dm') {
      ...
      runtimeLogs.push(`mapped legacy action "${intent?.action || ''}" -> "${action}"`);
    }

    let data;
    if (action === 'post_facebook') {
      data = await runPostFacebook(runtimeIntent, config, counters, runtimeLogs);
      return { success: true, data, error: null, metadata: withRuntimeMetadata(started, counters, null, runtimeLogs) };
    }
    ...
    if (action === 'post_instagram') {
      data = await runPostInstagram(runtimeIntent, config, counters, runtimeLogs);
      return { success: true, data, error: null, metadata: withRuntimeMetadata(started, counters, null, runtimeLogs) };
    }

    if (action === 'post_whatsapp') {
      data = await runPostWhatsApp(runtimeIntent, config, counters, runtimeLogs);
      return {
        success: true,
        data,
        error: null,
    ...
    if (action === 'create_campaign') {
      data = await runCreateCampaign(runtimeIntent, config, counters, runtimeLogs);
      return { success: true, data, error: null, metadata: withRuntimeMetadata(started, counters, null, runtimeLogs) };
    }

    throw new Error(`Unsupported action: ${action}`);
  } catch (error) {
    runtimeLogs.push(`error: ${String(error?.message || error || '')}`);
    return {
      success: false,
      data: null,
