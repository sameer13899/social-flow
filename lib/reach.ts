type ReachPlatform = "x" | "instagram" | "both";
type BasePlatform = "x" | "instagram";
...
type AnalyzeResult = Partial<Record<BasePlatform, PlatformAnalysis>>;
...
function capScore(value: number, max: number): number {
  return clamp(Math.round(value), 0, max);
}

function normalizePlatform(raw: string): ReachPlatform {
...
function normalizeAnalytics(input: AnalyzeInput): { followers: number; engagementRate: number } {
...
function detectMediaType(image?: string, imageMeta?: MediaMeta): "none" | "image" | "video" {
  const raw = String(
    imageMeta?.type
    || imageMeta?.kind
    || imageMeta?.format
...
function analyticsBonus(followers: number, engagementRate: number): number {
  let bonus = 0;
  if (followers >= 20000) bonus += 3;
  else if (followers >= 5000) bonus += 2;
  else if (followers >= 1000) bonus += 1;

  if (engagementRate >= 0.06) bonus += 2;
  else if (engagementRate >= 0.04) bonus += 1;

  return clamp(bonus, 0, 5);
}

function gradeForScore(score: number): { grade: string; label: string } {
  if (score >= 85) return { grade: "A", label: "High reach potential" };
  if (score >= 70) return { grade: "B", label: "Good, likely to perform" };
  if (score >= 55) return { grade: "C", label: "Average, needs work" };
  if (score >= 40) return { grade: "D", label: "Low reach expected" };
  return { grade: "F", label: "Rewrite recommended" };
}

function ratio(dimension: DimensionScore): number {
  return dimension.max > 0 ? dimension.score / dimension.max : 0;
}

function xHookScore(draft: string): DimensionScore {
  const first = firstWords(draft, 8).join(" ").toLowerCase();
  let score = 0;
  if (/^\d+/.test(first)) score += 6;
  if (/^(why|how|what|when|should|can|will)\b/.test(first)) score += 5;
  if (STRONG_VERBS.some((verb) => first.includes(verb))) score += 5;
  if (/\b(you|your)\b/.test(first)) score += 4;
  if (/(unpopular opinion|hot take|truth is|stop doing|nobody talks about|most people)/.test(first)) score += 10;
  if (/^(unpopular opinion|hot take)/.test(first)) score += 4;
...
function xOpinionScore(draft: string): DimensionScore {
...
function xFormatScore(draft: string): DimensionScore {
  const text = draft.toLowerCase();
  let score = 0;
  if (/\?/.test(text)) score += 5;
  if (/\b\d+[%x]?\b/.test(text)) score += 4;
  if (/\b\d+\s+(ways|steps|reasons|lessons|mistakes)\b/.test(text) || /(^|\n)\d+\./.test(text)) score += 4;
  if (/\b(i learned|i used to|yesterday|last week|story)\b/.test(text)) score += 3;
  if (/\b(unpopular opinion|hot take)\b/.test(text)) score += 5;
  return {
    key: "format",
    label: "Format",
...
function xReplyBaitScore(draft: string): DimensionScore {
  const text = draft.trim().toLowerCase();
  let score = 0;
  if (/\?$/.test(text)) score += 8;
  if (/\b(reply|comment|tell me|what do you think|agree|disagree|worth it)\b/.test(text)) score += 6;
  if (/\b(agree|disagree)\?$/.test(text)) score += 6;
  if (/\b(vs\.?|versus|better than|wrong|overrated|underrated)\b/.test(text)) score += 6;
  return {
    key: "reply_bait",
    label: "Reply Bait",
    score: capScore(score, 20),
...
function xLengthScore(draft: string): DimensionScore {
  const length = draft.trim().length;
  let score = 0;
  let note = "120-220 characters is usually the sweet spot.";
  if (length >= 120 && length <= 220) score = 10;
  else if (length >= 80 && length < 120) score = 7;
  else if (length > 220 && length <= 280) score = 7;
  else if (length > 280) {
    score = 2;
...
function xMediaScore(mediaType: "none" | "image" | "video"): DimensionScore {
  return {
    key: "media",
    label: "Media",
...
function instagramSaveScore(draft: string): DimensionScore {
  const text = draft.toLowerCase();
  let score = 0;
  if (/\b\d+\s+(ways|steps|rules|ideas|lessons|mistakes|frameworks)\b/.test(text) || /(^|\n)\d+\./.test(text)) score += 10;
  if (/\b(framework|system|blueprint|checklist|template|playbook)\b/.test(text)) score += 8;
  if (/\b(save this|bookmark this|come back to this)\b/.test(text)) score += 5;
  if (/\b(do this|try this|use this|follow these steps|actionable)\b/.test(text)) score += 4;
  return {
    key: "save",
...
function instagramHookScore(draft: string): DimensionScore {
  const first = draft.trim().slice(0, 125).toLowerCase();
  let score = 0;
  if (/^\d+/.test(first)) score += 6;
  if (/^[\p{Extended_Pictographic}\u2600-\u27bf]/u.test(draft.trim())) score += 5;
  if (/^(stop|how to|the truth|nobody tells you|before you|3 ways|5 mistakes)/.test(first)) score += 7;
  if (/\b(secret|mistake|instead of|nobody tells you|before you)\b/.test(first)) score += 7;
  return {
    key: "hook",
    label: "Hook",
    score: capScore(score, 20),
...
function instagramVisualScore(mediaType: "none" | "image" | "video"): DimensionScore {
  let score = 5;
...
function instagramHashtagScore(hashtagCount: number): DimensionScore {
  let score = 0;
  let note = "3-5 focused hashtags is the healthiest range.";
  if (hashtagCount === 0) note = "No hashtags detected.";
  else if (hashtagCount >= 3 && hashtagCount <= 5) score = 15;
  else if (hashtagCount >= 6 && hashtagCount <= 10) score = 9;
...
function instagramCtaScore(draft: string): DimensionScore {
  const text = draft.toLowerCase();
  let score = 0;
  if (/\b(save this|bookmark this)\b/.test(text)) score += 5;
  if (/\b(comment|reply|tell me)\b/.test(text)) score += 3;
  if (/\b(share|tag someone|send this)\b/.test(text)) score += 3;
  return {
    key: "cta",
    label: "CTA",
...
function instagramLengthScore(draft: string): DimensionScore {
  const length = draft.trim().length;
  let score = 3;
  ...
  } else if (length <= 300) score = 8;
  else if (length <= 1200) score = 10;
  else score = 6;
  return {
    key: "length",
    label: "Length",
...
function buildSuggestions(platform: BasePlatform, breakdown: DimensionScore[], mediaType: "none" | "image" | "video", hashtags: number): Suggestion[] {
...
function toIstParts(now = new Date()): { shifted: Date; minutes: number; dayIndex: number; label: string } {
...
function hoursUntilWindow(currentMinutes: number, startMinutes: number, live: boolean): number {
  if (live) return 0;
...
function getOptimalWindows(platform: BasePlatform, now = new Date()): TimingResult {
...
function getEngagementPlaybook(platform: BasePlatform, score?: number): PlaybookResult {
...
function analyzeX(input: AnalyzeInput, followers: number, engagementRate: number): PlatformAnalysis {
...
function analyzeInstagram(input: AnalyzeInput, followers: number, engagementRate: number): PlatformAnalysis {
  const draft = String(input.draft || "").trim();
  const mediaType = detectMediaType(input.image, input.imageMeta);
  const hashtags = countHashtags(draft);
  const breakdown = [
