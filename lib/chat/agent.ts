...
class AutonomousAgent {
  ...
  specialistFromText(text) {
    ...
    if (/\b(auth|token|webhook|debug|developer|scope|rate limit)\b/.test(raw)) return 'developer';
  ...
  async process(userInput) {
    ...
    const llm = this.llmCapability();
    if (!llm.ok) {
      const specialist = this.setActiveSpecialist('developer');
      const msg = `[${specialist.name}] ${this.missingApiKeyMessage(llm)}`;
      const suggestions = uniq([
        ...this.apiKeySuggestions(llm.provider),
    ...
    if (!decision) {
      try {
        ...
        const specialist = this.setActiveSpecialist('developer');
        const msg = `[${specialist.name}] AI planning failed: ${error.message}`;
        const suggestions = uniq([
          ...this.apiKeySuggestions(llm.provider),
          'Retry once the network/API endpoint is reachable.',
          ...this.proactiveSuggestionsFromContext()
        ]);
        this.context.addMessage('agent', msg);
        return {
  ...
  apiKeySuggestions(provider = 'openai') {
    const p = String(provider || 'openai');
    return [
      `Open Studio Settings -> API Keys & Credentials and set "${p}" provider + API key.`,
      `Or run: social agent setup --provider ${p} --api-key <key>`
    ];
  }
  ...
  async tryLlmDecision() {
    ...
    if (!parsed || typeof parsed !== 'object') {
      throw new Error('AI returned an invalid planning payload.');
    }
  ...
  async heuristicDecision(userInput) {
    ...
    return resolveIntentDecision({
      ...
      ambiguousSuggestions: ['Configure a valid API key: social agent setup --provider openai --api-key <key>']
    });
  }
